...
$Selection.TypeText("Azure Documentation Example")
...
## Add a table for VMs
$VMTable = $Selection.Tables.add($Word.Selection.Range, $VMs.Count + 2, 5,
...
Foreach ($VM in $VMs) {
    ...
    $VMTable.cell(($i+2),3).range.text = $VM.HardwareProfile.VmSize
    $VMTable.cell(($i+2),4).range.Bold = 0
    $VMTable.cell(($i+2),4).range.text = $VM.ResourceGroupName
    $VMTable.cell(($i+2),5).range.Bold = 0
    $VMTable.cell(($i+2),5).range.text = $NICLabel
